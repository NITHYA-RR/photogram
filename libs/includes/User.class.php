...
class User
{
    ...
    function __construct($value)
    {
        ...
        if (is_numeric($value)) {
            $sql = "SELECT `id`, `username` FROM `users` WHERE `id` = '$value' LIMIT 1";
        } else {
            $sql = "SELECT `id`, `username` FROM `users` WHERE `username` = '$value' LIMIT 1";
        }
        ...
        if ($result->num_rows) {
            ...
            $this->table = 'users'; // Assuming this is the table name
        } else {
